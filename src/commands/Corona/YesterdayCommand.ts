...
export default class YesterdayCommand extends Command {
  ...
  public async exec(message: Message, { country }: { country: string }): Promise<Message> {
    ...
    if(!check.length) return message.util!.send(new this.client.Embed(message, await this.client.guildsData.findOne({ id: message.guild!.id }).then(guild => guild.colour))
