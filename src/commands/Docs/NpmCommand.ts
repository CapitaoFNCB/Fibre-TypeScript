...
export default class NpmCommand extends Command {
  ...
  public async exec(message: Message, { query }: { query: string }) {
    await fetch(`https://registry.npmjs.org/${query}`).then(res => res.json()).then(body => {
      ...
      if(body.error || body.code) return message.util!.send(new this.client.Embed()
        .setDescription("No Package with this name")  
      )

      axios(`https://www.npmjs.com/package/${body.name}`).then(response => {
        ...
        let downloads = all.filter(x => x.length > 3).filter(x => x.includes("Downloads") &&  !x.includes("DownloadsWeekly")).toString().split("Version")[0].split("Downloads").filter(x => x.length).toString()
        let size = all.filter(x => x.length > 4 && x.includes("Size") || x.includes("Total")).length ? all.filter(x => x.length > 4 && x.includes("Size") || x.includes("Total"))[0].split("Size")[1] + all.filter(x => x.length > 4 && x.includes("Size") || x.includes("Total"))[1].split("Total")[0] : "`Unknown`"
        let files = all.filter(x => x.length > 4 && x.includes("Files")).length ? all.filter(x => x.length > 4 && x.includes("Files"))[0].split("Files")[1].match(/\d+/) : "`Unknown`"
        ...
        const embed = new this.client.Embed()
            ...
            .addField("This Weeks Downloads:", `\`${downloads}\``,true)
