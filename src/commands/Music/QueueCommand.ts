...
export default class QueueCommand extends Command {
  ...
  async exec (message: Message) {
    ...
    if(!player.queue.length){
        return message.channel.send(new this.client.Embed(message, await this.client.guildsData.findOne({ id: message.guild!.id }).then(guild => guild.colour)).setDescription("The queue is empty"));
    } else if(player.queue.length < 2) {
        return message.channel.send(new this.client.Embed(message, await this.client.guildsData.findOne({ id: message.guild!.id }).then(guild => guild.colour)).addField(`Now Playing`,`${player.queue[0].title}`, false));
    } else {
            ...
            collector.on("collect", async (collected: MessageReaction) => {
                switch (collected.emoji.name) {
                    case "⏪":
                        ...
                        let edit_Embed: Embed = new this.client.Embed(message, await this.client.guildsData.findOne({ id: message.guild!.id }).then(guild => guild.colour)).addField(`🎧 Now Playing`,`${player.queue[0].title}`, false)
                    ...
                    case "▶":
                        ...
                        if(!queuelist.length) return collected.users.remove(message.author);
                        collected.users.remove(message.author)
                        let next_embed: Embed = new this.client.Embed(message, await this.client.guildsData.findOne({ id: message.guild!.id }).then(guild => guild.colour)).addField(`🎧 Now Playing`,`${player.queue[0].title}`, false)
                            .addField(`Queue`, queuelist , false)
                            .setFooter(`Page: ${current} / ${pages} • ${player.trackRepeat ? "Repeating Track" : player.queueRepeat ? "Repeating Queue" : "Not Repeating"}`);
                    ...
                    case "⏩":
                        ...
                        if(!queuelist.length) return collected.users.remove(message.author);
                        collected.users.remove(message.author)
                        let end_embed: Embed = new this.client.Embed(message, await this.client.guildsData.findOne({ id: message.guild!.id }).then(guild => guild.colour)).addField(`🎧 Now Playing`,`${player.queue[0].title}`, false)
                            .addField(`Queue`, queuelist , false)
                            .setFooter(`Page: ${current} / ${pages} • ${player.trackRepeat ? "Repeating Track" : player.queueRepeat ? "Repeating Queue" : "Not Repeating"}`);
