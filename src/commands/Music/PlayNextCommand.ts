...
export default class PlayNextCommand extends Command {
  ...
  async exec (message: Message, { query }: { query: any }) {
    ...
    if(query == "This user is in the incorrect voice channel, connect to correct") return message.util!.send(new this.client.Embed(message, colour).setDescription(`You need to be in the same voice channel as me to use Playnext Command.`));
    let player: any;
