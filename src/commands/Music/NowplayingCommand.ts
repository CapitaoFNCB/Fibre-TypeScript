...
export default class NowplayingCommand extends Command {
  ...
  async exec (message: Message): Promise<Message> {
    ...
    if(!player.queue[0]) return message.channel.send(new this.client.Embed(message, colour).setDescription("There is nothing in the queue.")); 
