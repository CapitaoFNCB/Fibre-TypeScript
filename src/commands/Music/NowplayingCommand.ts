...
export default class NowplayingCommand extends Command {
  ...
  async exec (message: Message): Promise<Message> {
    ...
    if(!player){
      return message.util!.send(new this.client.Embed(message, await this.client.findOrCreateGuild({id: message.guild!.id}).then(guild => guild.colour)).setDescription("There is no player for this guild"))
    } else if(!channel || channel.id !== player.voiceChannel.id) {
      return message.channel.send(new this.client.Embed(message, await this.client.findOrCreateGuild({id: message.guild!.id}).then(guild => guild.colour)).setDescription("You need to be in the same voice channel as me to use Nowplaying Command")); 
    } else if(!player.queue[0]){
      return message.channel.send(new this.client.Embed(message, await this.client.findOrCreateGuild({id: message.guild!.id}).then(guild => guild.colour)).setDescription("There is nothing in the queue.")); 
    }
