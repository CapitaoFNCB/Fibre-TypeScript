...
export default class TagCommand extends Command {
    ...
    public async exec (message: Message) {
        ...
        if(player.queueRepeat) return message.channel.send(new this.client.Embed(message, colour).setDescription("Already looping queue."));
        player.setQueueRepeat(true)

        return message.util!.send(new this.client.Embed(message, colour).setDescription("Looping Queue"))
