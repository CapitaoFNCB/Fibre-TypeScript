...
export default class PlayNowCommand extends Command {
  ...
  async exec (message: Message, { query }: { query: any }) {
    ...
    if(query == "This user is in the incorrect voice channel, connect to correct") return message.util!.send(new this.client.Embed(message, colour).setDescription(`You need to be in the same voice channel as me to use Playnow Command.`));
    let player: any;
    ...
    if(data) {
        if(data.type == "playlist") {
            ...
            setTimeout(async () => {
              if(player.queue.length > counter){
                player.queue.add(player.queue[0], counter + 1);
                player.stop();
              }
        ...
        this.client.manager.search(query, message.author).then(async found => {
            switch (found.loadType) {
                ...
                case "PLAYLIST_LOADED":
                    ...
                    if(player.queue.length > found.playlist.tracks.length){
                      player.queue.add(player.queue[0], found.playlist.tracks.length + 1);
                      player.stop();
                    }
                    message.react(this.client.emojiList.reaction.accept).catch(() => null)
