...
export default class UserInfoCommand extends Command {
      ...
      args: [
        {
          ...
          type: async (message: Message, user: string) => {
            ...
            try {
              ...
              if(member && member.size) return member.first();
            } catch {}
            try {
              let resolved_user: any = await this.client.util.resolveUsers(user, this.client.users.cache);
              if(resolved_user && resolved_user.size) return resolved_user.first();
            } catch {}
            try {
  ...
  async exec (message: Message, { member }: { member: any }): Promise<Message> {
    ...
    return message.util!.send(new this.client.Embed(message, colour)
        ...
        .addField("Badges", member.flags.toArray().length ? member.flags.toArray().map(x => emojies[x]).join(" ") : "\`No Badges\`")
