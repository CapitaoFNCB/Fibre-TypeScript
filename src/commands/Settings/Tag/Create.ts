...
export default class TagCommand extends Command {
    ...
    public async exec (message: Message, { name, content }: { name: string; content: string; }) {
        ...
        if (name.match(USER_MENTION_REGEX) || name.match(CHANNEL_MENTION_REGEX))
            return message.util!.send(new this.client.Embed(message, await this.client.findOrCreateGuild({id: message.guild!.id}, this.client).then(guild => guild.colour)).setDescription(`You cannot name your tag that, as it contains a mention of a channel, or user.`));
